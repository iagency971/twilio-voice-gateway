...
def utc(x):
    z=pd.Timestamp(x)
    return z.tz_localize('UTC') if z.tzinfo is None else z.tz_convert('UTC')


def family_fields(s):
    sig=feat.family_signature(s)
...
def attach_identity(events,year):
    ...
    e['research_trading_date']=feat.xau_day_key(pd.to_datetime(e.contact_time,utc=True))
    ff=[family_fields(x) for x in e.constituent_families]
    e['signature']=[x[0] for x in ff];e['family_stack']=[x[1] for x in ff]
    return e


def parity_gate(events,canonical,sessions,year):
    dates=set(sessions.loc[sessions.acquisition_stage.eq('DEV_RANK1'),'research_trading_date'].astype(str))
    c=canonical[canonical.year.eq(int(year))].copy()
    c['research_trading_date']=feat.xau_day_key(pd.to_datetime(c.contact_time,utc=True))
    c=c[c.research_trading_date.isin(dates)].copy()
    e=events[events.research_trading_date.isin(dates)].copy()
    a=set(e.event_uid.astype(str));b=set(c.event_uid.astype(str))
    if a!=b:
        raise SystemExit(json.dumps({'error':'EVENT_UID_PARITY_FAIL','year':int(year),'generated':len(a),'canonical':len(b),'generated_only':sorted(a-b)[:5],'canonical_only':sorted(b-a)[:5]},indent=2))
...
def simulate_record(base,entry,bars_exec,target_r,commission,scenario,sc,risk_rule,vol_floor_k):
    ...
    z={
        'event_uid':base['event_uid'],'year':int(base['year']),'research_trading_date':base['research_trading_date'],
        'contact_time':base['contact_time'],'family_stack':base['family_stack'],'signature':base['signature'],
        'side':base.get('side'),'session':base.get('session'),'behavior_v2':base.get('behavior_v2'),
        'scenario':scenario,'scenario_role':sc['role'],'spread_usd':float(sc['spread_usd']),'commission_rt_usd':float(commission),
        'entry_model':base['_entry_model'],'risk_rule':risk_rule,'vol_floor_k':vol_floor_k,'target_r':float(target_r),
        'gross_R':float(sim['gross_R']),'net_R':float(sim['net_R_legacy22']),'result':sim['result'],
        'ambiguous_same_bar':bool(sim['ambiguous_same_bar']),'risk_price':float(entry['risk_price']),
...
def main():
    ...
    ap.add_argument('--canonical-events',required=True);ap.add_argument('--sessions',required=True);ap.add_argument('--out',required=True)
    a=ap.parse_args();start=utc(a.target_start);end=utc(a.target_end);year=int(start.year);out=Path(a.out);out.mkdir(parents=True,exist_ok=True)
    if end.year not in {year,year+1}:raise SystemExit('unexpected target window')
    cfg=ResearchConfig()

    canonical=pd.read_csv(a.canonical_events,compression='gzip',low_memory=False)
    sessions=pd.read_csv(a.sessions)
    bars_mid=load_ohlc_csv(a.csv).sort_index().copy();bars_mid['quote_active']=quote_activity_mask(bars_mid);bars_mid['sigma60']=robust_sigma60(bars_mid)
    zones=generate_baseline_zones(bars_mid,cfg);events=find_first_contacts(bars_mid,zones,bars_mid['sigma60'],cfg);events=collapse_contact_events(events,cfg.stack_overlap_threshold);events=label_contacts(bars_mid,events,cfg);events=classify_behavior_v2(bars_mid,events,cfg)
    if not events.empty:
    ...
    counts=[]
    if len(r):
        for key,g in r.groupby(['scenario','entry_model','risk_rule'],dropna=False):
            counts.append({'scenario':key[0],'entry_model':key[1],'risk_rule':key[2],'outcome_rows':int(len(g)),'unique_events':int(g.event_uid.nunique()),'rr_values':sorted(float(x) for x in g.target_r.unique())})
    manifest={
        'version':'COMEX_DEV_RANK1_VANTAGE_EVENT_OUTCOMES_V1','market_data_api_calls':False,'year':year,
        'canonical_selected_events':int(len(canon_sel)),'generated_selected_events':int(len(events)),'event_uid_parity':True,
        'selected_sessions':int(events.research_trading_date.nunique()),'outcome_rows':int(len(r)),'scenarios':SCENARIOS,
        'structural_models':STRUCTURAL_MODELS,'touch_next_open_vol_floor_k':list(TNO_K),'raw_touch_next_open_exported':False,
        'target_R_surface':[float(x) for x in TARGET_RS],'horizon_minutes':120,
        'engine':'unchanged rzr.build_entry + rzr.simulate_one + fixed Vantage overlay; event-level publication only',
        'net_r_surface_freeze':'COMEX_DEV_RANK1_NET_R_SURFACE_FREEZE_v1.md','entry_counts':entry_counts,'counts':counts,
    }
