...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('csv'); ap.add_argument('--target-start',required=True); ap.add_argument('--target-end',required=True); ap.add_argument('--out',required=True); ap.add_argument('--horizon-minutes',type=int,default=120); ap.add_argument('--sample',choices=['DISPLACEMENT_ORIGIN_ONLY','OBJECTIVE_LIQUIDITY_ONLY','MEMORY_ONLY','DOZ_OBJECTIVE_ONLY']); ap.add_argument('--model',choices=MODELS); a=ap.parse_args()
 ...
 if a.sample: samples={a.sample:samples[a.sample]}
 ...
 s=pd.DataFrame(rows); s.to_csv(out/'summary.csv',index=False); t.to_csv(out/'trade_surface.csv.gz',index=False,compression='gzip'); json.dump({'version':'PHASE_C_S1_VOLATILITY_FLOOR_V1','scientific_status':'exploratory sensitivity; exact k grid frozen before S1 run','target_start':str(start),'target_end':str(end),'target_events':len(contacts),'vol_floor_k':list(VOL_FLOOR_K),'target_R_surface':list(TARGET_RS),'entry_models':models,'samples':list(samples),'rule':'risk=max(structural_risk,k*sigma60); never tightens structural stop','costs':'$22 RT/100oz and 1.5x stress','entry_counts':counts},open(out/'manifest.json','w'),indent=2); print(s.sort_values('avg_net_R_legacy22',ascending=False).head(50).to_string(index=False) if len(s) else 'NO_TRADES')
