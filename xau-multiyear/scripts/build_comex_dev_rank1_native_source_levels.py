...
def main():
    ...
    for date in sorted(sessions.research_trading_date.astype(str)):
        ...
        for name,val0 in values.items():
            exact=float(val0);contact_tick=float(round(exact*10)/10)
            rows.append({'source_research_date':date,'source_session_start_utc':s.isoformat(),'known_time_utc':e.isoformat(),'terminal_leader':leader,'source_candidate_key':label,'source_instrument_id':iid,'level_type':name,'level_price':exact,'contact_tick_price':contact_tick,'primary_static_level':True})
    q=pd.DataFrame(rows);q.to_csv(out/'native_source_levels.csv',index=False)
    sm={'version':'COMEX_DEV_RANK1_NATIVE_SOURCE_LEVELS_V1_1','market_data_api_calls':False,'analytical_sessions':96,'source_sessions_with_primary_levels':int(q.source_research_date.nunique()) if len(q) else 0,'missing_source_sessions':missing,'levels_per_source':4,'levels_total':int(len(q)),'level_types':LEVELS,'contact_tick_rule':'round exact level_price to nearest GC 0.10 tick for first-contact detection; exact VWAP remains stored separately','contact_rule':'future primary contact requires first exact GC trade at contact_tick_price in next auction session on same raw instrument; no M1 crossing substitution','hvn_lvn_status':'not instantiated here; secondary definitions retained but local-extrema tie convention not expanded post-acquisition','outcomes_used_to_create_levels':False}
