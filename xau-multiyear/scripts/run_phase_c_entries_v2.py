...
from rzr.entries_v2 import build_entry, simulate_surface, TARGET_RS

FAMILIES = ['DISPLACEMENT_ORIGIN', 'OBJECTIVE_LIQUIDITY', 'MEMORY', 'FVG']
ENTRY_MODELS = ['PASSIVE_TOUCH', 'TOUCH_NEXT_OPEN', 'CLEAN_REJECTION', 'FAILED_AUCTION', 'RECLAIM_PULLBACK', 'ACCEPTANCE_RETEST']
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('csv'); ap.add_argument('--target-start',required=True); ap.add_argument('--target-end',required=True); ap.add_argument('--out',required=True); ap.add_argument('--horizon-minutes',type=int,default=120); args=ap.parse_args()
    ...
    outdir=Path(args.out); outdir.mkdir(parents=True,exist_ok=True); cfg=ResearchConfig()
    ...
    if missing: raise ValueError(f'Missing BID/ASK execution columns: {missing}')
    for c in required_exec: bars[c]=pd.to_numeric(bars[c],errors='raise')
    ...
    samples={'DISPLACEMENT_ORIGIN_ONLY':pure_doz,'OBJECTIVE_LIQUIDITY_ONLY':pure_obj,'MEMORY_ONLY':pure_mem,'DOZ_OBJECTIVE_ONLY':doz_obj}
    ...
    for sample,mask in samples.items():
        ...
        for model in ENTRY_MODELS:
            ...
            for rec in g.to_dict('records'):
                ...
                built+=1
                base={'sample':sample,'entry_model':model,'stack_id':rec.get('stack_id'),'zone_id':rec.get('zone_id'),'contact_time':rec.get('contact_time'),'behavior_v2':rec.get('behavior_v2'),'direction':entry['direction'],'entry_idx':entry['entry_idx'],'entry_price':entry['entry_price'],'stop_price':entry['stop_price'],'risk_price':entry['risk_price'],'buffer_price':entry['buffer_price'],'entry_delay_minutes':entry['entry_delay_minutes'],'sigma60':entry['sigma60']}
                for sim in simulate_surface(entry,bars,TARGET_RS,horizon_minutes=args.horizon_minutes): records.append({**base,**sim})
            entry_counts[f'{sample}:{model}']=int(built)

    trades=pd.DataFrame(records)
    if not trades.empty: trades['net_R_cost1_5x']=trades['gross_R']-1.5*trades['legacy_commission_R']
    rows=[]
    if not trades.empty:
        for (sample,model,tr),g in trades.groupby(['sample','entry_model','target_r'],sort=True):
            rows.append({'sample':sample,'entry_model':model,'target_r':float(tr),'trades':int(len(g)),'tp_pct':100.0*float((g.result=='TP').mean()),'sl_pct':100.0*float((g.result=='SL').mean()),'time_pct':100.0*float((g.result=='TIME').mean()),'ambiguous_same_bar_pct':100.0*float(g.ambiguous_same_bar.mean()),'avg_gross_R':float(g.gross_R.mean()),'pf_gross':float(pf(g.gross_R)),'avg_net_R_legacy22':float(g.net_R_legacy22.mean()),'pf_net_legacy22':float(pf(g.net_R_legacy22)),'avg_net_R_cost1_5x':float(g.net_R_cost1_5x.mean()),'pf_net_cost1_5x':float(pf(g.net_R_cost1_5x)),'median_risk_price':float(g.risk_price.median()),'median_entry_delay_minutes':float(g.entry_delay_minutes.median()),'median_legacy_commission_R':float(g.legacy_commission_R.median()),'sum_gross_R':float(g.gross_R.sum()),'sum_net_R_legacy22':float(g.net_R_legacy22.sum()),'sum_net_R_cost1_5x':float(g.net_R_cost1_5x.sum())})
    ...
    manifest={'source_commit':os.getenv('GITHUB_SHA','LOCAL'),'version':'PHASE_C_V2_ADDS_PREREGISTERED_RECLAIM_PULLBACK','target_start':str(start),'target_end':str(end),'bars':int(len(bars)),'zones_generated':int(len(zones)),'target_events':int(len(contacts)),'entry_models':ENTRY_MODELS,'target_R_surface':list(TARGET_RS),'horizon_minutes':int(args.horizon_minutes),'reclaim_pullback_rule':'after CLEAN_REJECTION or FAILED_AUCTION reclaim confirmation, limit at reclaimed proximal edge for max 15m; cancel if structurally invalidated before fill','stop_buffer_rule':'max(2 x contemporaneous spread, 0.10 x causal sigma60); R2 buffer fixed at confirmation','same_bar_tp_sl_rule':'SL / adverse resolution','execution':'Dukascopy M1 BID/ASK; Vantage $22 RT/100oz legacy commission sensitivity + 1.5x stress','overlap_policy':'signal-level study; overlapping candidate trades allowed; portfolio constraints deferred','entry_counts':entry_counts}
