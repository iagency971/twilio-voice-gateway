...
def build_entry(
    ...
    Entry models:
      TOUCH_NEXT_OPEN: benchmark, one active minute after contact.
      CLEAN_REJECTION: first proximal reclaim before distal breach, then next active open.
      FAILED_AUCTION: distal breach then proximal reclaim, then next active open.
      ACCEPTANCE_RETEST: preregistered accepted break, then first retest of distal edge.

    Market entries use ASK for longs and BID for shorts. Acceptance retests are limit
    entries at the numeric zone edge and require the executable side to reach the level.
    """
    n = len(bars)
    ...
    if behavior == "TOUCH_NEXT_OPEN":
        ...
        if str(rec.get("behavior_v2", "")) != "FAILED_AUCTION":
            return None
        ...
        direction = "LONG" if long_reject else "SHORT"
        if direction == "LONG":
            sweep_extreme = float(bars["low"].iloc[ci : confirm_i + 1].min())
        else:
            sweep_extreme = float(bars["high"].iloc[ci : confirm_i + 1].max())
    elif behavior == "ACCEPTANCE_RETEST":
        if str(rec.get("behavior_v2", "")) != "ACCEPTED_BREAK":
            return None
    ...
    if not np.isfinite(risk) or risk <= 0:
        return None
    return {
        "entry_model": behavior,
        "direction": direction,
        "contact_idx": ci,
        "confirm_idx": confirm_i,
        "entry_idx": ei,
        "entry_price": entry,
        "stop_price": float(stop),
        "risk_price": float(risk),
        "buffer_price": float(buf),
        "entry_delay_minutes": int(ei - ci),
        "sigma60": sig,
    }
...
def simulate_one(
    ...
    target_r,
    horizon_minutes=120,
    commission_rt_per_lot=22.0,
    contract_oz=100.0,
):
    """Simulate one trade using executable BID/ASK OHLC.

    Same-minute TP+SL ambiguity is resolved against the strategy (SL). A stop gapping
    beyond the stop price is filled at the worse opening executable quote. The legacy
    $22 round-turn/broker-lot commission is reported as a sensitivity only.
    """
    d = entry["direction"]
    ...
    if d == "LONG":
        hi = bars["high_bid"].to_numpy(float)
        lo = bars["low_bid"].to_numpy(float)
        op = bars["open_bid"].to_numpy(float)
        for j in range(ei, end):
            ...
            if tp:
                ...
                break
        if result == "TIME":
            exit_price = float(bars["close_bid"].iloc[exit_i])
        gross = (exit_price - ep) / risk
    else:
        hi = bars["high_ask"].to_numpy(float)
        lo = bars["low_ask"].to_numpy(float)
        op = bars["open_ask"].to_numpy(float)
        for j in range(ei, end):
            sl = hi[j] >= stop
            tp = lo[j] <= target
            if sl and tp:
            ...
            if tp:
                ...
                break
        if result == "TIME":
            exit_price = float(bars["close_ask"].iloc[exit_i])
        gross = (ep - exit_price) / risk

    commission_r = (
        float(commission_rt_per_lot) / (float(contract_oz) * risk)
    ...
    return {
        ...
        "gross_R": float(gross),
        "legacy_commission_R": float(commission_r),
        "net_R_legacy22": float(gross - commission_r),
    }
