...
def assign_stack_ids(zones: pd.DataFrame, overlap_threshold: float = 0.5) -> pd.DataFrame:
    """
    Descriptive zone-overlap tag only. It MUST NOT be used to collapse contacts,
    because static geometry across future zone creations would introduce lookahead.
    """
    if zones.empty:
        out = zones.copy()
        out["geometry_stack_hint"] = pd.Series(dtype=str)
        return out
    z = zones.sort_values(["known_time", "lower", "upper"]).reset_index(drop=True).copy()
    hints = [None] * len(z)
    counter = 0
    for known_time, idxs in z.groupby("known_time", sort=False).groups.items():
        ...
        for idx in idxs:
            ...
            for si, s in enumerate(stacks):
                overlap = max(0.0, min(row.upper, s["upper"]) - max(row.lower, s["lower"]))
                denom = min(width, s["upper"] - s["lower"])
                rel = overlap / denom if denom > 0 else 0.0
                if rel >= overlap_threshold:
                    assigned = si
                    s["lower"] = min(s["lower"], row.lower)
                    s["upper"] = max(s["upper"], row.upper)
                    break
            if assigned is None:
                assigned = len(stacks)
                stacks.append({"lower": row.lower, "upper": row.upper, "global": counter})
                counter += 1
            hints[idx] = f"GSTACK_{stacks[assigned]['global']:08d}"
    z["geometry_stack_hint"] = hints
    return z
...
def collapse_contact_events(contacts: pd.DataFrame, overlap_threshold: float = 0.5,
                            time_tolerance_minutes: int = 2) -> pd.DataFrame:
    """Causal event-level stacking with exact v1 first-match semantics.

    Contacts are processed in the same stable order as v1. Only groups touched
    within ``time_tolerance_minutes`` remain eligible. The expensive Python scan
    across every active group is replaced by a NumPy geometric prefilter; every
    surviving candidate is still verified with ``_relative_overlap`` in original
    group-creation order, so grouping semantics and STACK ids are unchanged.
    """
    if contacts.empty:
        out = contacts.copy()
        out["stack_id"] = pd.Series(dtype=str)
        return out
