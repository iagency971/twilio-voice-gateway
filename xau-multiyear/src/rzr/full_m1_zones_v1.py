...
def opening_quote_mask(bars: pd.DataFrame) -> pd.Series:
    """Quote availability known at the row open; never uses row H/L/C."""
    required = ("open_bid", "open_ask")
...
def first_open_quote_at_or_after(bars: pd.DataFrame, ts: pd.Timestamp) -> pd.Timestamp | None:
    mask = opening_quote_mask(bars).to_numpy(bool)
    pos = int(bars.index.searchsorted(pd.Timestamp(ts), side="left"))
    if pos >= len(mask):
        return None
    rel = np.flatnonzero(mask[pos:])
    if len(rel) == 0:
        return None
    return pd.Timestamp(bars.index[pos + int(rel[0])])
...
def causal_open_spread_series(bars: pd.DataFrame) -> pd.Series:
    ...
    bid = pd.to_numeric(bars["open_bid"], errors="coerce")
    ask = pd.to_numeric(bars["open_ask"], errors="coerce")
    s = (ask - bid).where(mask)
    return s.rename("open_spread")


def causal_point_half_width(
    bars: pd.DataFrame,
    sigma60: pd.Series,
    ...
    vals = np.maximum(2.0 * spread.to_numpy(float), config.point_zone_sigma_mult * sig.to_numpy(float))
    vals[~np.isfinite(spread.to_numpy(float)) | ~np.isfinite(sig.to_numpy(float))] = np.nan
...
def _metadata(
    *,
    ...
    information_time: pd.Timestamp,
    width_spread: float | None = None,
...
def _trading_day_start_utc(d, timezone: str) -> pd.Timestamp:
    local = pd.Timestamp(f"{d} 17:00:00", tz=timezone) - pd.Timedelta(days=1)
    return local.tz_convert("UTC")


def _week_start_utc(week_start_date, timezone: str) -> pd.Timestamp:
    # Trading-day week keyed Monday starts Sunday at the 17:00 NY trading-day boundary.
    local = pd.Timestamp(f"{week_start_date} 17:00:00", tz=timezone) - pd.Timedelta(days=1)
    return local.tz_convert("UTC")
...
def previous_period_levels_full_m1(
    bars: pd.DataFrame,
    sigma60: pd.Series,
    config: ResearchConfig,
) -> List[Zone]:
    ...
    for i in range(1, len(daily)):
        ...
        info_boundary = _trading_day_start_utc(cur[0], config.timezone)
        known = first_open_quote_at_or_after(bars, info_boundary)
        if known is None or known not in widths.index or pd.isna(widths.loc[known]):
            continue
        w = float(widths.loc[known])
        sp = float(open_spread.loc[known])
        for label, px, side in (
            ("PDH", prev[3], ZoneSide.RESISTANCE),
            ("PDL", prev[4], ZoneSide.SUPPORT),
        ):
            out.append(Zone(
                _make_zone_id(label, len(out)), ZoneFamily.OBJECTIVE_LIQUIDITY, label, side,
                prev[2], known, px - w, px + w, px, "D1",
                _metadata(
                    source_last=prev[2], information_time=known, width_spread=sp,
                    width_source="OPEN_BID_ASK_AT_KNOWN_TIME",
                    extra={"source_trading_date": str(prev[0])},
                ),
            ))

    week_keys = pd.Series(
        [pd.Timestamp(d).to_period("W-SUN").start_time.date() for d in day_key],
        index=active.index,
    )
    weekly = []
    ...
    for i in range(1, len(weekly)):
        ...
        info_boundary = _week_start_utc(cur[0], config.timezone)
        known = first_open_quote_at_or_after(bars, info_boundary)
        if known is None or known not in widths.index or pd.isna(widths.loc[known]):
            continue
        w = float(widths.loc[known])
        sp = float(open_spread.loc[known])
        for label, px, side in (
            ("PWH", prev[3], ZoneSide.RESISTANCE),
            ("PWL", prev[4], ZoneSide.SUPPORT),
        ):
            out.append(Zone(
                _make_zone_id(label, len(out)), ZoneFamily.OBJECTIVE_LIQUIDITY, label, side,
                ...
                _metadata(
                    source_last=prev[2], information_time=known, width_spread=sp,
                    width_source="OPEN_BID_ASK_AT_KNOWN_TIME",
                    extra={"source_week": str(prev[0])},
                ),
            ))
    return out


def completed_session_levels_full_m1(
    bars: pd.DataFrame,
    sigma60: pd.Series,
    config: ResearchConfig,
) -> List[Zone]:
    ...
    for key in instances:
        ...
        if g.empty:
            continue
        boundary = _session_end_utc(key, config.timezone)
        known = first_open_quote_at_or_after(bars, boundary)
        if known is None or known not in widths.index or pd.isna(widths.loc[known]):
            continue
        name = key[0]
        ...
        sp = float(open_spread.loc[known])
        common = dict(
            source_last=g.index[-1], information_time=known, width_spread=sp,
            width_source="OPEN_BID_ASK_AT_KNOWN_TIME",
            extra={"session_date": str(key[1])},
        )
        out.append(Zone(
            _make_zone_id(f"{name}_H", len(out)), ZoneFamily.OBJECTIVE_LIQUIDITY,
            f"{name}_HIGH", ZoneSide.RESISTANCE, g.index[-1], known,
            hi - w, hi + w, hi, "SESSION", _metadata(**common),
        ))
        out.append(Zone(
            _make_zone_id(f"{name}_L", len(out)), ZoneFamily.OBJECTIVE_LIQUIDITY,
...
def round_number_levels_full_m1(
    bars: pd.DataFrame,
    sigma60: pd.Series,
    config: ResearchConfig,
) -> List[Zone]:
    ...
    for ts, row in bars.loc[source_active].iterrows():
        ...
        if not np.isfinite(sig) or sig <= 0:
            continue
        known = first_open_quote_at_or_after(bars, pd.Timestamp(ts) + ONE_MINUTE)
        if known is None or known not in widths.index or pd.isna(widths.loc[known]):
            continue
        c = float(row.close)
        w = float(widths.loc[known])
            ...
            for px in (base - step, base, base + step):
                ...
                if abs(c - px) <= 3.0 * sig:
                    ...
                    out.append(Zone(
                        ...
                        f"ROUND_{step:g}", ZoneSide.NEUTRAL, ts, known,
                        px - w, px + w, px, "PRICE",
                        _metadata(
                            source_last=ts, information_time=known, width_spread=sp,
...
def directional_change_turns_full_m1(
    bars: pd.DataFrame,
    sigma60: pd.Series,
    config: ResearchConfig,
) -> pd.DataFrame:
        ...
        for i in pos:
            ...
            if mode >= 0:
                ...
                if extreme_price - l >= threshold:
                    known = first_open_quote_at_or_after(bars, ts + ONE_MINUTE)
                    if known is not None:
                        records.append({
                            "delta_mult": delta_mult, "kind": "HIGH",
                            "origin_time": extreme_time, "source_last_m1_timestamp_used": ts,
                            "information_available_time": known, "known_time": known,
                            "price": extreme_price, "reaction_amplitude": extreme_price - l,
                        })
                    mode = -1
                ...
                if h - extreme_price >= threshold:
                    ...
                    if known is not None:
                        records.append({
                            "delta_mult": delta_mult, "kind": "LOW",
                            "origin_time": extreme_time, "source_last_m1_timestamp_used": ts,
                            "information_available_time": known, "known_time": known,
                            "price": extreme_price, "reaction_amplitude": h - extreme_price,
                        })
                    mode = +1
                    extreme_price, extreme_time = h, ts
...
def dedupe_directional_turns_streaming(turns: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    """Earliest confirmation wins permanently; later scales cannot rewrite history."""
    if turns.empty:
        return turns, 0
...
def memory_zones_full_m1(
    ...
    if turns.empty:
        return [], {
            "memory_raw_turns": int(len(raw_turns)),
            "memory_deduped_turns": 0,
    ...
    for tr in turns.itertuples(index=False):
        ...
        if kt not in widths.index or pd.isna(widths.loc[kt]):
            continue
        w = float(widths.loc[kt])
        px = float(tr.price)
        left = bisect.bisect_left(spatial, (px - w, -1))
        right = bisect.bisect_right(spatial, (px + w, 10**18))
        best = None
        best_key = None
        ...
        reaction_amp = float(tr.reaction_amplitude)
        tr_kind = str(tr.kind)
        ...
        if best is None:
            ci = len(clusters)
            clusters.append({
                "center": px, "weight": max(reaction_amp, 1e-9), "count": 1,
            ...
            bisect.insort(spatial, (px, ci))
            continue
        cl = clusters[best]
        old_pair = (cl["center"], best)
        k = bisect.bisect_left(spatial, old_pair)
        if k < len(spatial) and spatial[k] == old_pair:
            spatial.pop(k)
        else:
            spatial.remove(old_pair)
        wt = max(reaction_amp, 1e-9)
        cl["center"] = (cl["center"] * cl["weight"] + px * wt) / (cl["weight"] + wt)
        cl["weight"] += wt
        cl["count"] += 1
        cl["known_time"] = kt
        cl["last_known_time"] = kt
        cl["kind_high"] += int(tr_kind == "HIGH")
        cl["kind_low"] += int(tr_kind == "LOW")
        cl["max_scale"] = max(cl["max_scale"], tr_scale)
        cl["source_last_m1_timestamp_used"] = source_last
        bisect.insort(spatial, (cl["center"], best))
        if cl["count"] == 2:
            side = (
                ZoneSide.RESISTANCE if cl["kind_high"] > cl["kind_low"]
                else ZoneSide.SUPPORT if cl["kind_low"] > cl["kind_high"]
                else ZoneSide.NEUTRAL
            )
            sp = float(open_spread.loc[kt])
            out.append(Zone(
                _make_zone_id("MEM", len(out)), ZoneFamily.MEMORY,
                "DIRECTIONAL_CHANGE_CLUSTER", side, cl["origin_time"], kt,
                cl["center"] - w, cl["center"] + w, cl["center"], "M1",
                _metadata(
                    source_last=source_last, information_time=kt, width_spread=sp,
                    width_source="OPEN_BID_ASK_AT_KNOWN_TIME",
    ...
    return out, {
        "memory_raw_turns": int(len(raw_turns)),
        "memory_deduped_turns": int(len(turns)),
        "memory_prefix_invariance_violations": int(prefix_viol),
    }
...
def fvg_zones_full_m1(bars: pd.DataFrame, config: ResearchConfig) -> List[Zone]:
    ...
    for i, ts in enumerate(bars.index):
        ...
        if low > float(hi2.loc[ts]):
            ...
            out.append(Zone(
                _make_zone_id("FVG_B", len(out)), ZoneFamily.FVG, "FVG_3BAR",
                ZoneSide.SUPPORT, bars.index[i - 2], known, lo, up, (lo + up) / 2, "M1",
                _metadata(source_last=pd.Timestamp(ts), information_time=known),
            ))
        if high < float(lo2.loc[ts]):
            lo, up = high, float(lo2.loc[ts])
            out.append(Zone(
                _make_zone_id("FVG_S", len(out)), ZoneFamily.FVG, "FVG_3BAR",
                ZoneSide.RESISTANCE, bars.index[i - 2], known, lo, up, (lo + up) / 2, "M1",
                _metadata(source_last=pd.Timestamp(ts), information_time=known),
            ))
    return out


def generate_baseline_zones_full_m1(
    bars: pd.DataFrame,
    config: ResearchConfig,
) -> tuple[List[Zone], dict]:
