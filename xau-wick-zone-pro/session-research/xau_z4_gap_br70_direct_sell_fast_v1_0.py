...
def diag(d):
 if len(d)==0:return {'executed':0,'terminal_n':0,'TP_FIRST':0,'INVALIDATION_FIRST':0,'NEITHER':0,'AMBIGUOUS':0,'terminal_tp_rate':None,'expectancy_R':None,'profit_factor_R':None,'nominal_rr':qpack([]),'stop_distance_v':qpack([]),'target_distance_v':qpack([])}
...
def main():
 a=parse_args();direct=load_module('gap_fast_helpers',Path('xau-wick-zone-pro/session-research/xau_e123_direct_sell_between_above_z4_v1_0.py'));v01=direct.v01
 raw=v01.load_raw(a.files);active=v01.active_m1(raw)
 # PERF ONLY: immutable timestamp arrays constructed once, unlike legacy raw_index helper.
 raw_ns=raw.time.to_numpy(dtype='datetime64[ns]');raw_i8=raw_ns.astype('int64')
 active_v={pd.Timestamp(t):float(v) for t,v in zip(active.time,active.v60) if np.isfinite(v) and v>0}
 ...
 states=[];state_for={}
 # Same causal map as v1_0: a Z4 snapshot at t applies strictly after t through t+5m.
 for t,g in z4.groupby('time',sort=True):
  ...
  tn=np.datetime64(t.tz_convert('UTC').tz_localize(None),'ns').astype('int64');t5n=np.datetime64((t+pd.Timedelta(minutes=5)).tz_convert('UTC').tz_localize(None),'ns').astype('int64')
  i0=int(np.searchsorted(raw_i8,tn,side='right'));i1=int(np.searchsorted(raw_i8,t5n,side='right')-1)
 ...
 for sid in sorted(sessions):
  ...
  for j in idxs:
   ...
   row=raw.loc[j];t=pd.Timestamp(row.time);v=active_v.get(t,np.nan)
 ...
 out={'status':'Z4_GAP_BR70_DIRECT_SELL_RETROSPECTIVE_EXPLORATORY_COMPLETE','session':a.session,'implementation':'PARITY_EQUIVALENT_FAST_V1_0','results':{}}
 for w,(lo,hi) in WINDOWS.items():
  q=tdf[(tdf.trigger_time>=lo)&(tdf.trigger_time<hi)] if len(tdf) else tdf;op=one_position(q);counts=q.groupby('session_id').size().to_numpy(float) if len(q) else np.array([])
