...
def data(n=400):
    rows=[]
    fams=['EPM_M1_R2_A8H','ESM_BOTH_G120M','EWM_G60M','ES_M1_8H_R2_T0.50']
    for i in range(n):
        w=.2+(i%40)/20.;p=1+(i%12);f=fams[i%4]
        latent=1.2*w+.12*np.log1p(p)+(i%4)*.08
        y=int((latent+(i%7)*.08)>1.55)
...
def run():
    ...
    b=me.bootstrap(sc,me.auc_assoc,n=100,seed=me.SEED)
    checks['cluster_bootstrap_requested_exact']=b['requested']==100 and b['valid']+b['invalid']==100
    q=me.quartile_rates(sc);checks['fixed_quartile_labels']=set(q)=={'Q1','Q2','Q3','Q4'} and all(q[x]['n']>0 for x in q)
    # Later unseen family maps to all-zero family indicators and is counted, not imputed.
    later=d.iloc[:20].copy();later['current_family']='UNSEEN_FAMILY';later_sc,lqa=me.transform_score(later,m)
    checks['unseen_family_counted']=lqa['unseen_family_rate']==1.0 and len(later_sc)==20
    # Missing model feature is excluded.
    miss=d.iloc[:10].copy();miss.loc[0,'zone_width_v']=np.nan;_,mqa=me.transform_score(miss,m)
    checks['missing_feature_excluded']=mqa['feature_excluded_rows']==1 and abs(mqa['feature_exclusion_rate']-.1)<1e-12
    # Complete session blocks are never split and sizes differ by at most one.
    blocks=me.prospective_blocks(sc);sizes=[b['session_n'] for b in blocks];flat=[s for b in blocks for s in b['sessions']]
    checks['prospective_blocks_complete_sessions']=len(flat)==len(set(flat))==sc.session_date_ny.nunique() and max(sizes)-min(sizes)<=1
