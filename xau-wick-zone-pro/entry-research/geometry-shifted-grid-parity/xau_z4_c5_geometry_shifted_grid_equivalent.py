...
def landmark_ok(ts):
    p=utc_ts(ts); return p.minute%5==0 and p.second==0
...
def main():
    ...
    base=(math.floor(L.min()/STEP)-100)*STEP; maxp=math.ceil(H.max()/STEP)*STEP; nlevels=int(round((maxp-base)/STEP))+16
    ...
    summ={'tag':args.tag,'rows':len(Z),'landmarks':int(Z.landmark_i.nunique()),'eligible_landmarks':int(len(eligible_landmarks)),
          ...
          'revisit_rate':None,'runtime_sec':time.time()-t0}
