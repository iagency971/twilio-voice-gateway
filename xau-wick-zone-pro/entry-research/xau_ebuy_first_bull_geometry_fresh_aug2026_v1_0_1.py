...
def bootstrap_auc(d, pred_col, sessions, seed=SEED, nrep=B):
    sessions = sorted(sessions)
    empty = d.iloc[:0]
    by = {s: d[d.session_id == s] if s in set(d.session_id) else empty for s in sessions}
...
def bootstrap_auc_diff(d, p1, p0, sessions, seed=SEED, nrep=B):
    sessions = sorted(sessions)
    empty = d.iloc[:0]
    by = {s: d[d.session_id == s] if s in set(d.session_id) else empty for s in sessions}
    rng = np.random.default_rng(seed)
...
def main():
    ...
    out = {
        ...
        "bootstrap_cluster_universe": "ALL_MECHANICALLY_ELIGIBLE_RAW_US_SESSIONS_INCLUDING_ZERO_EVENT_CLUSTERS",
        "engineering_correction": "ADDENDUM_A_BOOTSTRAP_CLUSTER_UNIVERSE",
    ...
    auc6_ci, auc6_nboot = bootstrap_auc(resolved, "geometry_score", complete)
    ...
    auc0_ci, auc0_nboot = bootstrap_auc(resolved, "closepos_score", complete)
    aucdiff = auc6 - auc0
    aucdiff_ci, aucdiff_nboot = bootstrap_auc_diff(resolved, "geometry_score", "closepos_score", complete)
